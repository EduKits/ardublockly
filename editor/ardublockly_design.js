...
/**
* Validade pin numbers in settings overlay
*/

$(document).ready(function () {
  //called when key is pressed in textbox
  $(".valid-pin").keypress(function (e) {
...
/* Sort out functions for kit selector */
$( ".button.select-kit" ).click(function() {
  $( ".kit-card > .options" ).toggleClass('selected', false);
  $( this ).parent().parent().addClass('selected');
});

/* Sort out functions for IO selector */
$( ".button.edit" ).click(function() {
  var topContent = $( this ).parent().parent().parent().parent();
  topContent.toggleClass( "open" ).toggleClass( "closed" );
});

$( ".button.check" ).click(function() {
  var topContent = $( this ).parent().parent().parent();

  topContent.find( ".pin-config" ).each(function(i, pinconfig) {
    var instance = $( this ).attr('data-instance');
    var content = topContent.find( '.valid-pin[data-instance="' + instance + '"]' ).val();
    if(content != '') $( this ).html( content );
  });

  var componentName = topContent.attr('data-component');

  switch(componentName) {
    case "buzzer":
      kitConfig.connections.buzzer = topContent.find( ".valid-pin" ).val();
      break;
    ...
    case "ultrasonic":
      topContent.find( ".pin-config" ).each(function(i, pinconfig) {
        var instance = $( this ).attr('data-instance');
        var content = topContent.find( '.valid-pin[data-instance="' + instance + '"]' ).val();
        if(instance == 0 && content != '') kitConfig.connections.ultrasonic.trig = content;
        if(instance == 1 && content != '') kitConfig.connections.ultrasonic.echo = content;
      });
      break;

    case "touch":
      kitConfig.connections.touch = topContent.find( ".valid-pin" ).val();
      break;
    ...
    case "colourLED":
      topContent.find( ".pin-config" ).each(function(i, pinconfig) {
        var instance = $( this ).attr('data-instance');
        var content = topContent.find( '.valid-pin[data-instance="' + instance + '"]' ).val();
        if(instance == 0 && content != '') kitConfig.connections.colourLED.red = content;
        if(instance == 1 && content != '') kitConfig.connections.colourLED.green = content;
        if(instance == 2 && content != '') kitConfig.connections.colourLED.blue = content;
      });
      break;
  }
